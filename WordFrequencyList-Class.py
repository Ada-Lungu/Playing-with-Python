...
def bubble_sort_alphabetically(my_list):
    ...
    while not_sorted_yet:
        ...
        for i in range(0, len(my_list)-1):
            j = i + 1
            if my_list[i].word > my_list[j].word:
...
def binary_search(freq_list, the_word):
    ...
    counter_searches = 0
    while left<=right:
        if the_word == freq_list[middle].word:
            return freq_list[middle].freq, counter_searches
        elif the_word < freq_list[middle].word:
            counter_searches +=1
            right = middle-1
            middle = (left + right) // 2
        elif the_word > freq_list[middle].word:
